...
typedef  struct  os_task_stk {
    ...
    BOOLEAN                Terminate;                               /* Task terminate flag.                                     */
    OS_TCB                    *OSTCBPtr;
} OS_TASK_STK;
...
/*
os_critcal_section define.
*/
CRITICAL_SECTION os_critical_section ;                          /* Critical section for OS_ENTER_CRITICAL/OS_EXIT_CRITICAL.  */
...
#if (CPU_CFG_CRITICAL_METHOD == OS_CRITICAL_METHOD)
    cpu_sr = 0u;
#endif
...
*              FALSE, current task NOT suspended.
*
...
* Arguments  : None.
*
* Returns    : TRUE,  current task     resumed successfully.
...
{
    // 在 Windows x64 上可以做临界区初始化，或者留空
    InitializeCriticalSection(&os_critical_section);
}
...
{
    DeleteCriticalSection(&os_critical_section);
}
